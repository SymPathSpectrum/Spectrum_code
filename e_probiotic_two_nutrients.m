...
GamR0 = 0.16;     %whether conspecific produces
GamRF = 0.0;
stpGamR = 1;

AbP0 = AbI/1000;        %path abundance 
AbPF = AbI;
stpA = 1;

kip0 = 3;       %probio affin for path
kipF = 0.05;
stpKip = 24;

kir0 = 0.05;        %probio affin for symbiont
kirF = 3;
stpKir = 24;
               ...
               for i = 1:1:(stpKip*2)+1
                   ...
                   kir = KirIndex(i);
                    ...
                    for GamI = GamI0:stpszG:GamIF
                        igi = igi +1;
                            eventfunc = @(t,y)e_NutrSteadyState(t,y,r,GamI,GamR,GamP,Degr,kn1,kpi,kpr,kip,krp,kri,kir,E,EP,HCE,kn2,Ov);
                            optionsode=odeset('Events',eventfunc,'NonNegative', 1:8);
                            y0 = [AbI AbR AbP AbTI AbTR AbTP Nu1 Nu2];
                            [t,y] = ode45(@(t,y)e_patch_mod(t,y,r,GamI,GamR,GamP,Degr,kn1,kpi,kpr,kip,krp,kri,kir,E,EP,HCE,kn2,Ov),tspan,y0,optionsode);
                            Vals = [t,y];
                            if Vals(end,2) > 1E-09 
...
%% finding the best parameters (when P is minimum)
%also finds the highest value of probiotic if there are mutliple instances
%of a minimum P value
        ...
        for gampcount = 1:1:stpGP+1
            ...
            elseif krpcount == 2
                ...
                Gam_finalabsP = Gam_MasterCellP{abcount + 2,gampcount};
                    ...
                    how_many = size(bestrow);
                    if how_many(1,1) > 1
                        warning = abcount,gampcount
                    end 
                    if krpcount == 1
