...
% AbI = AbR;
% AbP = AbI;
        invSigma = 1 - ((kip - 0.05)/(3-0.05));
        resSigma = 1 - ((krp - 0.05)/(3-0.05));
eventfunc = @(t,y) ot_3stp_NutrSteadyState(t,y,r,GamI,GamR,GamP,Degr,kn1,kpi,kpr,kip,krp,kri,kir,E,EP,HCE,nut);
optionsode=odeset('Events',eventfunc,'NonNegative', 1:7);
y0 = [AbI AbR AbP AbTI AbTR AbTP Nu1];
[t,y] = ode45(@(t,y)ot_3stp_patch_mod(t,y,r,GamI,GamR,GamP,Degr,kn1,kpi,kpr,kip,krp,kri,kir,E,EP,HCE,nut),tspan,y0,optionsode);
K = [t,y];
absI = y(:,1)';
absR = y(:,2)';
absP = y(:,3)';
