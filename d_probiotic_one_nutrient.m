...
GamI0 = 0.0;
GamIF = 0.99;
stpG = 99; %test these prod values

GamR0 = 0.16;     %conspecific either makes toxin or not
GamRF = 0.0;
stpGamR = 1;

AbP0 = AbI/1000;    %path abundance either low or high
AbPF = AbI;
stpA = 1;

kip0 = 3;       %probio affintity for path
kipF = 0.05;
stpKip = 24;

kir0 = 0.05;       %probio affnity for symbiont
kirF = 3;
stpKir = 24;
...
GamP0 = 0.0; %pathogen toxic or not
GamPF = 0.2;
stpGP = 1;
               ...
               for i = 1:1:(stpKip*2)+1
                   ...
                   kir = KirIndex(i);
                    ...
                    for GamI = GamI0:stpszG:GamIF
                        igi = igi +1;
                            eventfunc = @(t,y)d_NutrSteadyState(t,y,r,GamI,GamR,GamP,Degr,kn1,kpi,kpr,kip,krp,kri,kir,E,EP,HCE);
                            optionsode=odeset('Events',eventfunc,'NonNegative', 1:7);
                            y0 = [AbI AbR AbP AbTI AbTR AbTP Nu1];
                            [t,y,te,ye,ie] = ode45(@(t,y)d_patch_mod(t,y,r,GamI,GamR,GamP,Degr,kn1,kpi,kpr,kip,krp,kri,kir,E,EP,HCE),tspan,y0,optionsode);
                            Vals = [t,y];
                            if Vals(end,2) > 1E-09 
...
%% finding minimum P val

for krpcount = 1:1:stpGamR+1
    for abcount = 1:1:stpA+1
        for gampcount = 1:1:stpGP+1
            if krpcount == 1 %this bit just allows us to work in one matrix
            ...
            if krpcount == 1 %now find the lowest value of P achieved
        ...
        for gampcount = 1:1:stpGP+1
            ...
            for kipcount = 1:1:stpKip+1
                for gamcount = 1:1:stpG+1
                    ...
                    amount = size(pos);     %how many times minP reached
                    if amount(1,1) > 1  %if multiple minP, check them all to find highest probio
